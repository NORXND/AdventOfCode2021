...
def foldX(foldPos: int, table: Table) -> Table:
    ...
    # Deviding into left
    right_table = create_new_table(maxX-foldPos, len(table))
    for rowI, row in enumerate(right_table):
        for colI, col in enumerate(row):
            if colI < maxX-foldPos:
                col.value = new_table[rowI][(foldPos+1)+colI].value
...
fold = folds[0]
if fold["Type"] == "X":
    folded = foldX(fold["Value"], original_table)
elif fold["Type"] == "Y": 
    folded = foldY(fold["Value"], original_table)
